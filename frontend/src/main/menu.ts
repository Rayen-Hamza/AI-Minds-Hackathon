...
export function getMainAppMenu(): Menu {
  ...
  windowMenu?.submenu?.append(
    new MenuItem({
      label: "Always Show Klippy on Top",
      type: "checkbox",
      checked: getStateManager().store.get("settings").klippyAlwaysOnTop,
      click: (menuItem) => {
        getStateManager().store.set(
          "settings.klippyAlwaysOnTop",
          menuItem.checked,
        );
      },
    }),
  ...
  windowMenu?.submenu?.append(
    new MenuItem({
      ...
      type: "checkbox",
      checked: getStateManager().store.get("settings").minimizeToTray,
      click: (menuItem) => {
        getStateManager().store.set(
