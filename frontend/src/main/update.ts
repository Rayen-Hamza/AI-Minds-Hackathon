...
export async function checkForUpdates() {
  if (!app.isPackaged) {
    return dialog.showMessageBox({
      ...
      title: "Update Check",
      message:
        "You are running a development version of Klippy, so the auto updater is disabled.",
    });
  }
  ...
  try {
    ...
    if (isUpdateAvailable) {
      ...
      await dialog.showMessageBox({
        ...
        message: `${comparisonString} You are already using the latest version of Klippy.`,
      });
    }
  } catch (error) {
