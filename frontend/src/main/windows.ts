...
import { hideToTray, updateTrayMenu } from "./tray";
...
export async function createMainWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    thickFrame: false,
    title: "Klippy",
    alwaysOnTop: settings.klippyAlwaysOnTop,
    webPreferences: {
  ...
  // Handle close event to hide to tray if enabled
...
function isChatWindow(window: BrowserWindow): boolean {
  return window.webContents.getTitle() === "Klippy Chat";
}
...
export function setFont(
  ...
  windows.forEach((window) => {
    window.webContents.executeJavaScript(
      `document.querySelector('.klippy').setAttribute('data-font', '${font}');`,
    );
  });
}
