...
export const Chats: React.FC<SettingsProps> = ({ onClose }) => {
  ...
  const chatsWithPreview = Object.values(chatRecords).map((chat) => ({
    ...
    preview: chat.preview || "(Empty chat)",
  }));
  ...
  const chatCount = chatsWithPreview.length;
    ...
    >
      <div className="chats-header">
        <div>
          ...
          <span className="helper-text">
            {chatCount} {chatCount === 1 ? "conversation" : "conversations"} saved
          </span>
        </div>
      ...
      {chatCount === 0 ? (
        ...
        <TableView
          ...
          style={{ height: "calc(80vh - 120px)", overflow: "auto" }}
